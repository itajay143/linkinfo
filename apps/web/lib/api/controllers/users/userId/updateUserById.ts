...
import { AccountSettings } from "@linkinfo/types/global";
import bcrypt from "bcrypt";
import { removeFile, createFile, createFolder } from "@linkinfo/filesystem";
import sendChangeEmailVerificationRequest from "@/lib/api/sendChangeEmailVerificationRequest";
import { i18n } from "next-i18next.config";
import { UpdateUserSchema } from "@linkinfo/lib/schemaValidation";
...
export default async function updateUserById(
  ...
  if (user && user.email && data.email && data.email !== user.email) {
    ...
    sendChangeEmailVerificationRequest(
      user.email,
      data.email,
      data.name?.trim() || user.name || "Linkinfo User"
    );
  }
